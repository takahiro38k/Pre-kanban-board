...
import { reorderPatch, sortBy } from './util'
...
export type State = {
  ...
  columns?: {
    id: ColumnID
    title?: string
    // 他のstateから独立させることで、不要なcomponentsの再renderを防ぐ。
    // text?: string
    cards?: {
      id: CardID
      text?: string
...
export const reducer: Reducer<State, Action> = produce(
  (draft: State, action: Action) => {
    switch (action.type) {
      ...
      // case 節がすべての action.type を網羅していることを、
      // 型検査で検出するためのテクニックです（ためしに case 節をどれか 1 つ
      // コメントアウトすると意味がわかります）
      default: {
